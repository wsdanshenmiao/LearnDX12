...
namespace DSM {
	bool ObjectManager::AddObject(std::shared_ptr<Object> object) noexcept
	{
		if (object == nullptr || m_Objects.contains(object->GetName())) {
			...
			if (object->GetModel() != nullptr) {
				object->SetCBIndex(m_Objects.size());
			}
	...
	ID3D12Resource* ObjectManager::CreateObjectsResource(ID3D12Device* device, UINT CBSize)
	{
		std::size_t objModelCount = 0;
		for (const auto&[name, object] : m_Objects) {
			if (auto model = object->GetModel();model != nullptr) {
				objModelCount++;
			}
		...
		resourceDesc.Width = CBByteSize * objModelCount;
