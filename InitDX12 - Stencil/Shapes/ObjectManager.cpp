...
namespace DSM {
	...
	bool ModelManager::IsChange() const noexcept
	{
		return m_IsChange;
	}

	void ModelManager::AddMesh(std::shared_ptr<Object> obj)
	{
	...
	std::size_t ModelManager::GetObjectSize() const noexcept
	{
		return m_Objects.size();
	}

	std::shared_ptr<Object> ModelManager::GetMesh(const std::string& name)
	{
	...
	std::shared_ptr<Object> ModelManager::GetObjectByIndex(const std::size_t& index)
	{
		return m_Objects[index];
	}

	Geometry::GeometryMesh ModelManager::GetAllObjectMesh() const
	{
		GeometryMesh mesh{};
