...
#undef ISBITMASK
    ...
    HRESULT CreateTextureResource(
        ...
        _Outptr_ D3D12_RESOURCE_DESC& textureDesc) noexcept
    {
        if (!d3dDevice)
            return E_POINTER;
    ...
    HRESULT CreateTextureFromDDS(_In_ ID3D12Device* d3dDevice,
                                 ...
                                 D3D12_RESOURCE_FLAGS resFlags,
                                 DDS_LOADER_FLAGS loadFlags,
                                 _Outptr_ D3D12_RESOURCE_DESC& textureDesc,
                                 ...
                                 _Out_opt_ bool* outIsCubeMap) noexcept(false)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = CreateTextureResource(d3dDevice, resDim, twidth, theight, tdepth, reservedMips - skipMip, arraySize,
                format, resFlags, loadFlags, textureDesc);
    ...
    bool* isCubeMap)
{
    ...
    hr = CreateTextureFromDDS(d3dDevice,
        ...
        resFlags, loadFlags,
        textureDesc, subresources, isCubeMap);
    if (SUCCEEDED(hr)&& alphaMode)
    {
