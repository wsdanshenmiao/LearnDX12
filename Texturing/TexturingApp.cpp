...
namespace DSM {
	TexturingApp::TexturingApp(HINSTANCE hAppInst, const std::wstring& mainWndCaption, int clientWidth, int clientHeight)
		:D3D12App(hAppInst, mainWndCaption, clientWidth, clientHeight) {
	}

	bool TexturingApp::OnInit()
	{
	...
	void TexturingApp::OnUpdate(const CpuTimer& timer)
	{
	...
	void TexturingApp::OnRender(const CpuTimer& timer)
	{
	...
	bool TexturingApp::InitResource()
	{
		// 为创建帧资源，因此使用基类的命令列表堆
	...
	void TexturingApp::CreateShader()
	{
	...
	/// 创建几何体
	/// </summary>
	void TexturingApp::CreateGeometry()
	{
	...
	void TexturingApp::CreateTexture()
	{
		// 读取并创建纹理
	...
	void TexturingApp::CreateLights()
	{
		auto& lightManager = LightManager::GetInstance();
	...
	void TexturingApp::CreateFrameResource()
	{
	...
	void TexturingApp::CreateMaterial()
	{
	...
	void TexturingApp::CreateDescriptorHeaps()
	{
		// 创建纹理的描述符堆
	...
	void TexturingApp::CreateRootSignature()
	{
	...
	void TexturingApp::UpdateFrameResource(const CpuTimer& timer)
	{
		auto& imgui = ImguiManager::GetInstance();
	...
	void TexturingApp::UpdateGeometry(const CpuTimer& timer)
	{
		auto& imgui = ImguiManager::GetInstance();
	...
	const std::array<const D3D12_STATIC_SAMPLER_DESC, 6> TexturingApp::GetStaticSamplers() const noexcept
	{
		// 创建六种静态采样器
